...
class CloudAccountService:
        ...
        try:
            ...
            for row in rows:
                ...
                account_dict = {
                    ...
                    'accountInfo': json.loads(account['account_info']) if account['account_info'] else {},
    ...
    def get_decrypted_credentials(self, account_id):
        ...
        try:
            cursor = conn.cursor()
            cursor.execute('SELECT encrypted_credentials FROM cloud_accounts WHERE id = ?', (account_id,))
            row = cursor.fetchone()
            
            if not row or not row[0]:
                return None
            
            decrypted_data = self.cipher_suite.decrypt(row[0].encode())
